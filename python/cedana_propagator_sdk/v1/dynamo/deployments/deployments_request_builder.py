...
class DeploymentsRequestBuilder(BaseRequestBuilder):
    """
    Builds and executes requests for operations under /v1/dynamo/deployments
    """
    def __init__(self,request_adapter: RequestAdapter, path_parameters: Union[str, dict[str, Any]]) -> None:
        ...
        Returns: None
        """
        super().__init__(request_adapter, "{+baseurl}/v1/dynamo/deployments?cluster_id={cluster_id}{&namespace*}", path_parameters)
    
    def by_name(self,name: str) -> WithNameItemRequestBuilder:
        """
        Gets an item from the cedana_propagator_sdk.v1.dynamo.deployments.item collection
        param name: DGD name
    ...
    async def get(self,request_configuration: Optional[RequestConfiguration[DeploymentsRequestBuilderGetQueryParameters]] = None) -> Optional[list[DynamoDeploymentView]]:
        """
        Returns all DynamoGraphDeployments synced from the specified cluster.
        param request_configuration: Configuration for the request such as headers, query parameters, and middleware options.
        Returns: Optional[list[DynamoDeploymentView]]
        """
        ...
        return await self.request_adapter.send_collection_async(request_info, DynamoDeploymentView, error_mapping)
